...
\newcommand{\codice}[4]{\lstinputlisting[language={C},firstline={#1}, lastline={#2},caption={#3},label={#4},frame = none]{"D:/UNIFI/Magistrale/Tesi/spectre/primeProbe/spark.c"}}
\newcommand{\codspark}[3]{
	\begin{center}
		\lstinputlisting[language={C},firstline={#1},lastline={#2},firstnumber={#1},frame={none},caption={[#3]}]{code/spark.c}
	\end{center}}
\newcommand{\arr}[2]{$\text{\emph{#1}}\left[ \text{\emph{#2}}\right]$}
\newcommand{\arrdoppio}[3]{$\text{\emph{#1}}\left[ \text{\emph{#2}} \left[\text{\emph{#3}}\right]\right]$}
...
\lstset{ %
	...
	frame=none %rlbt,	         	% adds a frame around the code
...
\emph{"From camp to camp, through the foul womb of night,\\The hum of either army stilly sounds,\\That the fixed sentinels almost receive\\The secret whispers of each other’s watch." \break --- Henry V, William Shakespear\\} 
\null\vspace{10mm}
